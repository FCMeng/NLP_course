...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.activation = torch.relu     #relu非线性函数
        self.loss = nn.CrossEntropyLoss()  #loss函数交叉熵
...
def evaluate(model, vocab, sample_length):
    ...
    for item in y:
        if item == 0:
            ...
            count_2 += 1
    print(f"本次预测集中共有{count_0}个0样本，{count_1}个1样本, {count_2}个2样本")
    correct, wrong = 0, 0
    with torch.no_grad():
        y_pred = model(x)      #模型预测
        softmax = nn.Softmax()
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print(f"输入：{input_string}, 预测类别：{output[i].argmax()}") #, 概率值：%f" % (input_string, round(float(result)), result)) #打印结果
